...
class Network(object):
    ...
    def update_parameters(self, learning_rate, alpha, beta):
        ...
        assert(learning_rate or (alpha and beta))
        ...
        if learning_rate:
            ...
            if not self.VdW and not self.Vdb:
                self.VdW = copy.deepcopy(self.dW)
                self.Vdb = copy.deepcopy(self.db)
            for layer in range(1, self.L + 1):
                self.VdW[layer] = (
                    beta * self.VdW[layer]
                    + (1 - beta) * self.dW[layer]
                )
                self.Vdb[layer] = (
                    beta * self.Vdb[layer]
                    + (1 - beta) * self.db[layer]
                )
                self.weights[layer] = (
                    self.weights[layer] - alpha * self.VdW[layer]
                )
                self.biases[layer] = (
                    self.biases[layer] - alpha * self.Vdb[layer]
                )
    ...
    def run_gradient_descent(self, X, Y, epochs, learning_rate=None,
        ...
        for i in range(epochs):
            ...
            self.update_parameters(learning_rate, alpha, beta)
            progress_bar.update()
        return costs
