...
def main():
    ...
    heading('Training vectors (33):')
    pp.pprint(temp)

    heading('Training vectors (33-interpolated):')
    pp.pprint(X_train[:, 32])
    ...
    heading('PCA training vectors:')
    pp.pprint(X_train_pca)
    ...
    plt.show()
    pp.pprint(lr.predict(X_test_pca))
    exit('TEST')

    heading('Test vectors (33interpolated):')
    pp.pprint(X_test[:, 32])
